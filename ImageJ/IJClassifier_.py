...
DEFAULT_OUTPUT_PATH = "/Users/ubcg83a/Desktop/test/"
...
class IJClassifier_(object):
	...
	def dialog(self):
		...
		dlg.addChoice("Image file:",image_titles,"None")
		dlg.addChoice("Mask file:",image_titles,"None")
		dlg.addCheckbox("Exclude edges", True)
		dlg.addCheckbox("Zip classifier", True)
		...
		dialog_options = {'window_size': dlg.getNextNumber(), 'data': dlg.getNextChoice(),
						  'mask': dlg.getNextChoice(), 'edges': dlg.getNextBoolean(),
						  'zip': dlg.getNextBoolean(), 'save': dlg.getNextBoolean(),
						  'path': path_listener.path}
	
		# check that we actually selected an image file
		if dialog_options['data'] == "None":
	...
	def write(self, ROIs=None, dialog_options=None):
		...
		# get the data stack
		data_stack = WindowManager.getImage( dialog_options['data'] )

		# get the mask stack
		if not dialog_options['mask'] == None:
			mask_stack = WindowManager.getImage( dialog_options['mask'])
		else:
			mask_stack = None
		...
		for r in ROIs:
			...
			r(data=data_stack, mask=mask_stack)
...
class ClassifierROI(object):
	...
	Notes:
		None
	"""
	def __init__(self, ROI=None, path=None):
	...
	def __call__(self, data=None, mask=None):
		...
		for i in xrange(len(self)):
			...
			new_im.setTitle(self.__ROI.name+"_"+str(i))
			self.write(new_im)
			...
			if not isinstance(mask, ImagePlus):
				continue

			new_im = self.grab(mask, temp_ROI)
			new_im.setTitle("mask_"+self.__ROI.name+"_"+str(i))
			self.write(new_im)
